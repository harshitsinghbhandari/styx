...
export async function buildGraph(pool: Pool, rootId: string): Promise<Graph> {
  const { rows: upEdges } = await pool.query<DependencyEdgeRow>(
    `WITH RECURSIVE up AS (
       SELECT commitment_id, depends_on_id FROM commitment_dependencies WHERE commitment_id = $1
       UNION
       SELECT cd.commitment_id, cd.depends_on_id FROM commitment_dependencies cd
       JOIN up ON cd.commitment_id = up.depends_on_id
     )
     SELECT commitment_id, depends_on_id FROM up`,
    [rootId],
  );
  const { rows: downEdges } = await pool.query<DependencyEdgeRow>(
    `WITH RECURSIVE down AS (
       SELECT commitment_id, depends_on_id FROM commitment_dependencies WHERE depends_on_id = $1
       UNION
       SELECT cd.commitment_id, cd.depends_on_id FROM commitment_dependencies cd
       JOIN down ON cd.depends_on_id = down.commitment_id
     )
     SELECT commitment_id, depends_on_id FROM down`,
    [rootId],
  );
  ...
  for (const e of [...upEdges, ...downEdges]) {
    ...
    seenEdges.add(key);
    edges.push({ from: e.commitment_id, to: e.depends_on_id });
  }
