#!/usr/bin/env bash
# Resets local CockroachDB and runs Scene 4 (Crash) for recording.
...
echo "--- running scene4-crash ---"
cd "$ROOT"
DATABASE_URL="$DB_URL" npx tsx scripts/scenes/scene4-crash.ts

echo "=== STYX DEMO: Scene 4 complete ==="
